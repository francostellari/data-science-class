...
spacex_df['labels'] = spacex_df['class'].map(labels)
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        selected = spacex_df[spacex_df['Launch Site'] == entered_site].groupby(['class'])['class'].count()
        selected = selected.to_frame()
        fig = px.pie(selected, values='class', 
        names=selected.index, 
        title=f'Site {entered_site}')
        return fig
...
def get_scatter_chart_chart(entered_site, payload_range):
